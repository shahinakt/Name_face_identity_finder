...
@app.get("/")
async def root():
    return {"message": "Name Face Identity Finder API", "status": "running", "version": "1.0.0"}

@app.get("/health")
async def health_check():
    return {"status": "healthy", "service": "name-face-identity-finder", "endpoints": ["/search", "/fast-search", "/search-stream", "/test-search"]}
...
async def search(name: str = Form(None), file: UploadFile = File(None), use_enhanced: bool = Form(False)):
    try:
        ...
        # Use optimized search with enhanced parameter
